...
# 4. Распределение функциональности, ограничения и качественные характеристики

В данном разделе функциональные требования, сформулированные в Итерации 3, распределяются по контейнерам и компонентам логической архитектуры. На основе этого распределения выявляются архитектурные инсайты, формулируются ограничения системы и определяются нефункциональные требования (качественные характеристики), скорректированные с учётом оценки нагрузки из Пункта 2.

---

## 4.1. Матрица распределения функциональности по компонентам

| Функциональный блок | Ответственный контейнер | Внутренние компоненты | Зависимости (внутр./внеш.) | Обоснование размещения |
|:---|:---|:---|:---|:---|
| **Регистрация и KYC** | Core API | `Auth Service`, `Auth Repository` | DB, Redis, OAuth Provider | Логика аутентификации и верификации требует сильной консистентности и хранения сессий. OAuth делегирован внешнему провайдеру. |
| **Каталог и поиск серверов** | Core API + Cache | `Catalog Service`, `Catalog Repository` | Redis, DB | Read-heavy операция. Данные нод кешируются в Redis, фильтрация происходит в памяти API Gateway/Core API для минимальной задержки. |
| **Бронирование и Эскроу** | Core API | `Booking Service`, `Escrow Service`, `Booking Repository` | DB, Payment Gateway, Message Broker | Критично к целостности. Холдирование и смена статусов заказа выполняются в рамках одной ACID-транзакции в PostgreSQL. |
| **Поминутный биллинг и выплаты** | Async Workers | `Billing Worker`, `Scheduler` | Message Broker, DB, Payment Gateway | Фоновая задача с высокой частотой тиков. Вынесена из синхронного пути, чтобы не блокировать UI и не создавать пиковую нагрузку на БД. |
| **Приём телеметрии и heartbeat** | Agent Gateway | `Stream Receiver`, `Metrics Validator` | Node Agent, Redis, Message Broker | Write-heavy поток (~1000 msg/sec). Выделен в отдельный stateless-контейнер для изоляции нагрузки и предотвращения деградации основного API. |
| **Управление сессиями (Start/Stop)** | Agent Gateway + Core API | `Command Dispatcher` | Message Broker, Node Agent | Команды маршрутизируются через брокер к конкретному агенту. Асинхронное подтверждение выполнения защищает от таймаутов. |
| **Личные кабинеты и аналитика** | Web App / Admin Panel | `UI Components`, `Data Fetchers` | Core API, Redis | Представленческий слой. Запрашивает агрегированные данные из API, который отдаёт их из кэша или материализованных view. |
| **Модерация и арбитраж** | Admin Panel + Core API | `Arbitration Module` | DB, Agent Gateway (логи телеметрии) | Требует доступа к истории транзакций и сырым метрикам нод для объективного разрешения споров. |
| **Уведомления и чеки** | Async Workers | `Notification Dispatcher` | Message Broker, Notification Service | Fire-and-forget паттерн. Гарантированная доставка через очередь, устойчивость к падению внешнего SMTP/SMS-шлюза. |

## 4.2. Архитектурные инсайты и выявленные пробелы

Распределение функциональности позволило выявить несколько ключевых архитектурных закономерностей и точек роста:

1. **Чёткое разделение Sync/Async путей:** Все операции, влияющие на UX (поиск, бронирование, получение SSH-ключей), идут по синхронному пути с минимальной латентностью. Длительные или ненадёжные операции (биллинг, уведомления, вебхуки платёжки) вынесены в асинхронный контур через Message Broker. Это устраняет риск блокировки потоков и таймаутов на клиенте.
2. **Изоляция Write-Heavy нагрузки:** Телеметрия от Node Agent генерирует ~1000 сообщений/сек при 1000 активных нодах. Если бы этот трафик шёл в Core API или напрямую в БД, это вызвало бы contention на уровне соединений и дискового I/O. Выделение `Agent Gateway` + асинхронная запись в Redis/брокер полностью развязало эти потоки.
3. **Финансовая целостность vs Масштабируемость:** Модуль эскроу намеренно оставлен в `Core API` с прямым доступом к PostgreSQL. Распределённые транзакции (Saga) на этапе MVP избыточны и несут риски рассинхронизации балансов. ACID-гарантии реляционной БД критичнее горизонтального масштабирования для финансовых операций.
4. **Выявленный пробел (Monitoring & Backup):** В исходной матрице отсутствовали компоненты observability. Добавлены требования к интеграции с Prometheus/Grafana (RED-метрики, бизнес-KPI) и автоматизированному бэкапу PostgreSQL (WAL-архивация + ежедневные снапшоты в S3).
5. **Готовность к эволюции:** Модульные границы в Go-монолите (`Auth`, `Catalog`, `Booking`) изолированы через интерфейсы. При росте нагрузки `Billing` или `Telemetry` могут быть вырезаны в отдельные микросервисы без переписывания ядра, так как коммуникация уже идёт через брокер событий.

## 4.3. Ограничения системы

| Категория | Ограничение | Влияние на архитектуру и разработку |
|:---|:---|:---|
| **Технологические** | MVP реализуется как модульный монолит (Golang + React + PostgreSQL). Оркестрация контейнеров на стороне провайдеров — только Docker. | Упрощает CI/CD, деплой и отладку. Исключает сложность service mesh и распределённого трейсинга на старте. |
| **Бизнес-модель** | Комиссия платформы фиксируется на уровне 5–10%. Работа только с фиатными валютами. Платформа не несёт ответственности за физическое железо провайдера. | Упрощает биллинг-логику. Требует чётких пользовательских соглашений (ToS) и механизмов автоматического возврата средств при сбоях на стороне хоста. |
| **Регуляторные** | Соответствие 152-ФЗ (хранение ПДн в РФ), PCI DSS (обработка платежей), KYC для вывода средств. | Запрет на хранение полных данных карт. Шифрование чувствительных полей в БД. Обязательная верификация перед payout. Выбор дата-центров в юрисдикции РФ. |
| **Скоуп MVP** | Отсутствуют: мобильные приложения, автомасштабирование кластеров, криптовалютные платежи, сложная предиктивная аналитика. | Фокус ресурсов на ядре: эскроу, каталог, телеметрия, базовый арбитраж. Все отложенные функции задокументированы как `v2.0+ Roadmap`. |
| **Интеграционные** | Зависимость от SLA внешних сервисов (платёжный шлюз, Email/SMS, OAuth). | Реализация паттернов Circuit Breaker, Retry с экспоненциальной задержкой, fallback-сценариев и очередей для сглаживания пиков недоступности. |

## 4.4. Качественные характеристики (NFR) с учётом нагрузки (Пункт 2)

Нефункциональные требования сформулированы на основе оценки пропускной способности, объёмов данных и сценариев обмена из Пункта 2.

| Характеристика | Требование | Метрика / SLA | Архитектурная реализация |
|:---|:---|:---|:---|
| **Доступность (Availability)** | Система должна быть доступна для бронирования и управления сессиями в режиме 24/7. | `SLA 99.9%` (~43 мин допустимого простоя/мес) | Stateless-контейнеры за балансировщиком, health-checks, автоперезапуск, master-replica репликация БД, graceful degradation при падении внешних шлюзов. |
| **Надёжность (Reliability)** | Отсутствие потери финансовых данных и статусов заказов. Гарантия доставки команд агентам. | `Data Loss < 0.01%`, `Exactly-once billing` | ACID-транзакции в PostgreSQL, идемпотентность платежей (`idempotency_key`), персистентные очереди в Broker, подтверждение ack от Node Agent. |
| **Производительность (Performance)** | Быстрый отклик UI и стабильная обработка телеметрии без деградации API. | `API Latency ≤ 200 мс (p95)`<br>`Ingest Throughput ≥ 1000 msg/sec`<br>`Payment Processing ≤ 5 сек` | Чтение каталога из Redis, пакетная запись метрик, асинхронный биллинг, connection pooling к БД, HTTP/2 для gRPC-стримов. |
| **Масштабируемость (Scalability)** | Возможность горизонтального роста под нагрузку без переписывания ядра. | `Linear scaling до 5000 нод и 2000 RPS API` | Горизонтальное масштабирование Core API и Agent Gateway, шардирование очередей по partition key (`node_id`), read-replicas БД для аналитики. |
| **Расширяемость (Extensibility)** | Быстрое подключение новых платёжных систем, типов метрик или провайдеров инфраструктуры. | `Time-to-integrate new payment gateway ≤ 2 недели` | Интерфейсы адаптеров для шлюзов, схема метрик в JSONB/Protobuf с backward compatibility, plugin-архитектура Agent SDK. |
| **Безопасность (Security)** | Защита ПДн, финансовых данных и изоляция задач арендаторов на железе провайдера. | `PCI DSS Level 1 compliant`<br>`Zero host access for tenants` | TLS 1.3 везде, mTLS/JWT для агентов, RBAC, шифрование полей в БД, запуск задач в изолированных Docker-контейнерах (опционально gVisor), rate limiting на Gateway. |

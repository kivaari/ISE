...
# 5. Сценарии с ошибками и пограничные случаи (Error & Edge Cases)

В данном разделе рассматриваются ситуации, которые не входят в «счастливый путь» (Happy Path), но неизбежно возникают в реальной эксплуатации P2P-маркетплейса. Проработка этих сценариев демонстрирует отказоустойчивость архитектуры и зрелость модели данных.

---

## 5.1. Сценарий 1: Сбой платёжного шлюза при холдировании (Инфраструктурная ошибка)

**Контекст:** Клиент нажимает «Оплатить», но платёжный шлюз (ЮKassa/Stripe) отвечает таймаутом или возвращает ошибку 5xx.
**Затронутые компоненты:** Web App, Core API (Booking Service), Message Broker, Async Workers, Payment Gateway.

### Поток обработки ошибки

| Шаг | Событие | Действие системы | Сущность БД |
|:---|:---|:---|:---|
| 1 | Клиент подтверждает бронь | Core API создаёт заказ со статусом `pending` и генерирует `idempotency_key`. | `bookings` (INSERT, status=`pending`) |
| 2 | Core API вызывает платёжный шлюз | Отправляется запрос на холдирование. Шлюз не отвечает (таймаут 5 сек). | — |
| 3 | Срабатывает Circuit Breaker | Core API не блокирует поток пользователя, а возвращает ответ: «Платёж в обработке». | — |
| 4 | Асинхронная постановка в очередь | Core API публикует событие `PaymentInitiated` в Message Broker. | — |
| 5 | Retry-логика (Async Worker) | Worker пытается отправить запрос в шлюз 3 раза с экспоненциальной задержкой (10с, 30с, 60с). | — |
| 6а | **Успех после retry** | Шлюз ответил. Worker обновляет статус заказа на `active`. | `escrow_transactions` (INSERT), `bookings` (UPDATE) |
| 6б | **Окончательный отказ** | Шлюз вернул ошибку «Недостаточно средств» или «Отказано банком». | — |
| 7 | Откат состояния (6б) | Worker меняет статус заказа на `cancelled`, публикует событие для уведомлений. | `bookings` (UPDATE status=`cancelled`) |
| 8 | Уведомление клиента | Клиент получает Push/Email: «Оплата не прошла, попробуйте другую карту». | — |

### Обоснование архитектурных решений
* **Идемпотентность:** Если сеть «моргнула» и клиент нажал кнопку дважды, `idempotency_key` в `escrow_transactions` гарантирует, что списание произойдет только один раз.
* **Circuit Breaker + Queue:** Пользовательский интерфейс не «висит». Если шлюз лежит, система не накапливает очередь из зависших HTTP-соединений, а честно ставит задачу в брокер и уведомляет клиента позже.

---

## 5.2. Сценарий 2: Открытие спора (Dispute) между клиентом и провайдером (Бизнес-ошибка)

**Контекст:** Клиент утверждает, что сервер не соответствовал заявленным характеристикам (например, GPU не той модели или постоянные лаги), и требует возврат средств. Провайдер утверждает, что задача была запущена и ресурсы потреблялись.
**Затронутые компоненты:** Web App, Core API (Arbitration Module), Admin Panel, Primary DB.

### Поток обработки спора

| Шаг | Событие | Действие системы | Сущность БД |
|:---|:---|:---|:---|
| 1 | Клиент нажимает «Открыть спор» в ЛК | Система замораживает эскроу-транзакцию. Биллинг останавливается. | `bookings` (UPDATE status=`dispute`), `escrow_transactions` (UPDATE status=`frozen`) |
| 2 | Автоматический сбор доказательств | Async Worker делает snapshot всех `metrics_snapshots` за период сессии и прикрепляет к спору. | `disputes` (INSERT), `evidences` (INSERT, type=`telemetry`) |
| 3 | Уведомление сторон | Клиент и Провайдер получают уведомление о начале разбирательства. | — |
| 4 | Анализ администратором | Админ открывает панель арбитража. Видит: заявленные характеристики (`hardware_specs`) vs фактические метрики (`metrics_snapshots`). | Чтение из БД |
| 5а | **Решение: Возврат клиенту** | Админ подтверждает, что GPU не соответствовал. | `resolutions` (INSERT, decision=`refund`) |
| 5б | **Решение: Выплата провайдеру** | Админ видит, что метрики в норме, клиент просто передумал. | `resolutions` (INSERT, decision=`none`) |
| 5в | **Решение: Частичный возврат** | Компромисс (например, 50/50). | `resolutions` (INSERT, decision=`partial`, amount=...) |
| 6 | Исполнение решения | Worker вызывает Payment Gateway для возврата/списания, обновляет балансы `wallets`. | `wallets` (UPDATE), `escrow_transactions` (UPDATE) |
| 7 | Пересчёт репутации | Если вина провайдера доказана, `interruption_rate` растёт, рейтинг падает. | `reputations` (UPDATE) |

### Обоснование архитектурных решений
* **Связь Billing ↔ Telemetry:** Благодаря внешнему ключу `telemetry_snapshot_id` в `billing_records`, администратор может в один клик увидеть, **что именно происходило на сервере в ту минуту, когда списались деньги**. Это делает арбитраж объективным и снижает время.resolution до KPI ≤ 24ч.
* **Заморозка эскроу:** Статус `frozen` в БД физически не позволяет воркерам биллинга случайно списать или вывести средства до решения админа.

---

## 5.3. Сценарий 3: Отказ в регистрации Node Agent (Безопасность / Фрод)

**Контекст:** Злоумышленник пытается подключить чужой сервер или сервер с вредоносным ПО, чтобы перехватывать команды или майнить.
**Затронутые компоненты:** Node Agent, Agent Gateway, Core API, Redis.

### Поток обработки нарушения

| Шаг | Событие | Действие системы | Сущность БД |
|:---|:---|:---|:---|
| 1 | Агент отправляет `RegisterRequest` | Agent Gateway проверяет `auth_token` (JWT, выданный при генерации скрипта в ЛК провайдера). | — |
| 2 | Проверка токена | Токен невалиден (истёк TTL > 1 часа или сгенерирован вручную). | — |
| 3 | Отказ в регистрации | Agent Gateway возвращает ошибку `UNAUTHENTICATED`, соединение закрывается. | — |
| 4 | Фиксация инцидента | Gateway публикует событие `SecurityAlert` в очередь. | `security_logs` (INSERT, если таблица ведется) |
| 5 | Блокировка IP (опционально) | При 5 неудачных попытках с одного IP — временный бан на уровне Nginx/Traefik. | Redis (Blacklist) |
| 6 | Уведомление провайдера | Если токен был валиден, но использован с незнакомого IP, провайдеру идёт алерт: «Попытка подключения с нового IP». | — |

### Обоснование архитектурных решений
* **mTLS / JWT с коротким TTL:** Скрипт установки выдаёт одноразовый токен. После первой успешной регистрации агент получает постоянный, но отзывной сертификат. Это исключает риск подбора токенов брутфорсом.
* **Изоляция Agent Gateway:** Даже если хакер начнёт флудить запросами регистрации, он «положит» только шлюз телеметрии, но не затронет Core API и возможность клиентов бронировать серверы.

---

## 5.4. Сводная матрица отказоустойчивости (Resilience Matrix)

| Сценарий ошибки | Триггер | Механизм защиты в архитектуре | Влияние на пользователя | Время восстановления |
|:---|:---|:---|:---|:---|
| **Сбой Платёжного шлюза** | Timeout / 5xx от API | Circuit Breaker, Async Queue, Retry with Backoff | Задержка уведомления об оплате, но заказ не теряется | Автоматически (секунды/минуты) |
| **Двойной вебхук оплаты** | Сеть продублировала запрос | `idempotency_key` (UNIQUE constraint в БД) | Никакого (процесс идёт штатно) | 0 мс (отклоняется БД) |
| **Потеря связи с Node Agent** | Отсутствие heartbeat > 60 сек | Детект в Agent Gateway, автосмена статуса в Redis, алерт | Клиент уведомлён, биллинг остановлен, средства возвращены | < 2 минуты |
| **Спор (Dispute)** | Жалоба клиента/провайдера | Заморозка эскроу (`status=frozen`), snapshot телеметрии | Средства заморожены до решения админа | ≤ 24 часа (SLA) |
| **Попытка взлома / Фрод** | Невалидный токен агента | JWT/mTLS в Agent Gateway, Rate Limiting | Атакующий заблокирован, провайдер уведомлён | Мгновенно |
| **Падение PostgreSQL** | Hardware failure / OOM | Master-Slave репликация, автоматический Failover | Кратковременная недоступность чтения/записи | < 30 секунд (auto-failover) |

# 6. Сценарии предполагаемого развития системы (Evolution Scenarios)
...
## 6.1. Сценарий 1: Переход к Big Data и глубокой аналитике телеметрии (v2.0)
...
## 6.2. Сценарий 2: Внедрение динамического ценообразования и AI-рекомендаций (v2.5)
...
## 6.3. Сценарий 3: Поддержка кластеров и оркестрации (Kubernetes) (v3.0)
...
## 6.4. Сценарий 4: Мобильное приложение и расширенные Push-уведомления (v2.0)
...
## 6.5. Сводная матрица эволюции системы
